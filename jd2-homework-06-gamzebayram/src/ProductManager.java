...
public class ProductManager {
	...
	public double calculateAveragePriceInCategory(String category) {
		...
		return products.values().stream()
				.filter(product -> product.getCategory().equals(category)).mapToDouble(Product::getPrice).average().orElse(0.0);
	...
	public Map<String, Double> getCategoryPriceSum() {
		...
		return products.values().stream().collect(Collectors.groupingBy(
				Product::getCategory, Collectors.summingDouble(product -> product.getPrice() * product.getStock())));
